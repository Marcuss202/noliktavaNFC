...
# Whitelist of sort keys -> concrete model order_by fields.
ORDER_SORT_FIELDS = {
...
class OrderListView(StaffOnlyMixin):
    """
    List all orders (staff only), with optional sorting.
    GET /api/orders?sort=address  (prefix with '-' for descending, e.g. '-created_at')
    """
...
class OrderDetailView(StaffOnlyMixin):
    ...
    PATCH /api/orders/<id>  Body: {"status": "shipped"}
    """
...
class AccountEmailsView(APIView):
    """
    List existing account emails so the checkout form can restrict orders to real accounts.
